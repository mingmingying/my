...
void MergeSort(int *a, int left, int right, int *tmp)
{
	...
	int mid = left + ((right - left) >> 1);//mid将数组二分
	MergeSort(a, left, mid, tmp);	//左边合并排序,使得左子序列有序
	MergeSort(a, mid + 1, right, tmp);	//右边合并排序,使得右子序列有序	
	_Merge(a, left, mid, mid + 1, right, tmp);//将两个有序子数组合并
}
//打印数组
